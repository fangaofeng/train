...
function EditExamPlan(props) {
  ...
  const { data: learnplan, loading } = learnplanService.retriveRequest(studyPlanID);
  const columns = [
    {
      title: '培训群组编号',
      dataIndex: 'group_no',
  ...
  return (
    <PageHeaderWrapper title="查看考试计划">
      <Spin spinning={loading}>
        <CourseBasicInfo isShow ExamInfo={learnplan?.course} />
      </Spin>
      <Card className={styles.testInfoDetail}>
        <Descriptions title="考试计划信息" bordered>
          <Descriptions.Item label="考试名称">{learnplan.name}</Descriptions.Item>
          <Descriptions.Item label="开放时间">{learnplan.start_time}</Descriptions.Item>
          <Descriptions.Item label="结束时间">{learnplan.end_time}</Descriptions.Item>
        </Descriptions>
        <ProTable
          columns={columns}
          request={async (params = {}) => {
            const { data } = await request(() => learnplanService.getGroups(studyPlanID), {
              params,
            });
            return {
              data: data.results,
        ...
        <ProTable
          ...
          request={async (params = {}) => {
            const { data } = await request(
              () => learnplanService.getGroupMembers(studyPlanID, group.no),
              {
                params,
              },
            );
