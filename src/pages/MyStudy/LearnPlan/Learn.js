...
export default function Learn(props) {
  ...
  const progressInfo = useSelector(store => store.MyLearnPlan.progressInfo);
  const progressInfoloading = useSelector(
    store => store.loading.effects['MyLearnPlan/GetLearnProgressDetail']
  );
  ...
  useEffect(() => {
    storedispatch({
      type: 'MyLearnPlan/GetLearnProgressDetail',
      payload: {
        id,
      },
    });
