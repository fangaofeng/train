...
import { TreeSelect , Modal , Card , Button , Table , Divider , Popconfirm , Icon , Upload , Row , Col , message , Input , Select , Form , Avatar } from 'antd';
...
class UserManager extends Component {
    ...
    // 单个删除
    deleteConfirm = (id) => {
      const { dispatch , allUsersTableData } = this.props;
      ...
      const len = allUsersTableData.results.length;// 获取该页数据条数
      // dispatch({
      ...
      //           },
      //         });
      //         this.getAllTableData(prePage,pageSize);
      //       }else{
      //         this.getAllTableData(current,pageSize);
      //       }
      //     }else{
          ...
          },
          {
            ...
            render: (text, record) => (
              <span>
                ...
                <Popconfirm placement="topRight" title='确认删除这个用户吗？' onConfirm={() => this.deleteConfirm(record.key)} okText="确定" cancelText="取消">
                  <a>删除</a>
                </Popconfirm>
              </span>
            ),
          ...
          <PageHeaderWrapper title='用户管理'>
            <Card className={styles.userManagerContent}>
              <div className={styles.searchContent}>
                ...
                <div className=''>
                  <Button type='primary' onClick={this.goPage}>批量导入用户</Button>
                  <Button type='primary'>增加用户</Button>
                  <Button className='ant-btn-del' onClick={this.batchDelete}>批量删除用户</Button>
                </div>
              </div>
              <Table 
            ...
            <Modal
              ...
              footer={
                <div style={{textAlign:'center'}}>
                  <Button type="primary" loading={confirmLoading} onClick={this.handleOk}>确定</Button>
                  <Button onClick={this.handleCancel}>取消</Button>
                </div> 
              }
            >
              <div style={{display:'flex',justifyContent:'center',alignItems:'center',padding:'40px 0'}}>
                <Icon type="exclamation-circle" theme="filled" style={{color:'#faad14', fontSize:'24px',marginRight:'15px'}} />确认删除这些用户吗？
              </div>
            </Modal>
          </PageHeaderWrapper>
        );
