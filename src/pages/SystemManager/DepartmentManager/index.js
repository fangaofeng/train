...
class DepartmentManager extends Component {
  ...
  GetAlreadyOrgs = () => {
    ...
    dispatch({
      ...
      callback: res => {
        if (res.status === 'ok') {
          // console.log('GetOrgsDeparments success', res);
          ...
          console.log('GetOrgsDeparments 失败', res);
        }
      },
    });
  };
  ...
  render() {
    ...
    const { treeData, importcount, alreadyExistTreeData } = this.state;
    // const previewLoop = data =>
    //   data.map(item => {
    //     if (item.children) {
    //       return (
    //         <TreeNode key={item.id} title={item.name}>
    //           {previewLoop(item.children)}
    //         </TreeNode>
    //       );
    //     }
