...
import { Card , Table , Row , Col , Input , Avatar , Button , message , Modal , Icon } from 'antd';
...
@connect(({ uploadCourse }) => ({
  ...
  tableData:uploadCourse.tableData,// 获取指定页码的表格数据
}))
class UploadZip2 extends Component {
  ...
  getTableData = (page,size) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'uploadCourse/GetTrainerTableData',
      payload: {
        page,// 页码
        size,// 每页条数
      },
  ...
  render() {
    ...
    const dataSource = tableData.results.map((value)=>{
        ...
        <Card className={styles.afterUpload}>
          <Row gutter={24} className={styles.afterUploadRow}>
            <Col xs={24} sm={24} md={24} lg={24} xl={12} xxl={12} className={styles.firstCol}>
              <SelfCard title='课程信息' bordered={false} noLeftAndRightPadding='noLeftAndRightPadding'>
                <div className={styles.imgContent}>
                  <div className={styles.imgLeft}>
                    <img src={zipInfo.KJFM} alt="" />
                  </div>
                  <div className={styles.imgRight}>
                    <div className={styles.msgDetail}><span>课件编号：</span><span className={styles.msgDetailOverflow} title={zipInfo.KJBH}>{zipInfo.KJBH}</span></div>
                    <div className={styles.msgDetail}><span>课件名称：</span><span className={styles.msgDetailOverflow} title={zipInfo.KJMC}>{zipInfo.KJMC}</span></div>
                    <div className={styles.msgDetail}><span>课时：</span><span>{zipInfo.KS}小时</span></div>

                  </div>
                </div>
                <Row style={{marginTop:15}}>
                  <Col>
                    <div className={styles.msgDetail}><span>适用对象：</span><span>{zipInfo.SYDX}</span></div>
                  </Col>
                </Row>
                <div className={styles.columnContent}>
                  <div className={styles.columnContentL}>
                    <div className={styles.msgDetail}><span>课件分类：</span><span>{zipInfo.KJFL == '0' ? '公开课':'非公开课'}</span></div>
                  </div>
                ...
                <div className={styles.columnContent}>
                  <div className={styles.columnContentL}>
                    <div className={styles.msgDetail}><span>课程文件类型：</span><span>{zipInfo.KJWJLX == '0' ? 'PDF':'MP4'}</span></div>
                  </div>
                  <div className={styles.columnContentR}>
                    <div className={styles.msgDetail}><span>允许视频拖放：</span><span>{zipInfo.YXSPTF == '0' ? '是':'否'}</span></div>
                  </div>
                </div>
                <Row>
                  <Col>
                    <div className={styles.msgDetail} style={{lineHeight:1.5}}><span>课件介绍：</span><span>{zipInfo.KJJS}</span></div>
                  </Col>
                </Row>
              </SelfCard>
              <SelfCard title='讲师信息' bordered={false} noLeftAndRightPadding='noLeftAndRightPadding'>
                <div className={styles.imgContent2}>
                  ...
                  <div className={styles.imgRight2}>
                    <div className={styles.msgDetail}><span>讲师姓名：</span><span>{zipInfo.JSXM}</span></div>
                    <div className={styles.msgDetail}><span>讲师介绍：</span><span>{zipInfo.JSJS}</span></div>
                  </div>
                </div>
              </SelfCard>
            </Col>
          ...
          <div className={styles.footerBtns}>
            <Button type="primary" onClick={this.saveTableData}>保存</Button>
        ...
        <Modal
          ...
          footer={
            <div style={{textAlign:'center'}}>
              <Button type="primary" onClick={this.submitModal}>确定</Button>
            </div> 
          }
        >
          <div style={{display:'flex',justifyContent:'center',alignItems:'center',padding:'20px 0'}}>
            <Icon type="check-circle" theme="filled" style={{color:'#8fc127', fontSize:'40px',marginRight:'15px'}} /><span style={{fontSize:'20px',color:'#3b3b3b'}}>课件已保存</span>
          </div>
          <div style={{padding:'0 0 20px 0',textAlign:'center',fontSize:'16px',color:'#3b3b3b'}}>您可在课件管理页中查看已保存课件！</div>
        </Modal>
      </PageHeaderWrapper>
