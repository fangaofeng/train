...
class CommonConent extends Component {
  ...
  getTableData = (page, size) => {
    const { dispatch, ID } = this.props;
    dispatch({
      type: 'CourseManager/GetTrainmanagers',
      payload: {
        id: ID, // 课件id
        page, // 页码
        size, // 每页条数
  ...
  getAddTableData = (page, size) => {
    const { dispatch, ID } = this.props;
    dispatch({
      type: 'CourseManager/GetOthertrainmanagers',
      payload: {
        page, // 页码
        size, // 每页条数
        id: ID, // 培训群组ID
  ...
  render() {
    ...
    const dataSource = currenttrainmanagers.results.map(value => Object.assign({}, value, { key: value.id }));
    ...
    const addDataSource = addtrainmanagers.results.map(value => Object.assign({}, value, { key: value.id }));
    // ---------------增加培训管理员模态框---------------
    ...
    return (
      <PageHeaderWrapper title={`课件编辑（${currentType}）`}>
        ...
        <Card className={styles.courseManagerContent}>
