...
import { Card , Button , Table , Icon , message , Input , Modal , Popconfirm } from 'antd';
...
class CommonConent extends Component {
    ...
    getTestPapersInfo=()=>{
      ...
      dispatch({
        type: 'ExamManager/GetPaperDetail',
        payload: {
          id:ID,// id
        },
        callback: (res)=>{
    ...
    getTableData = (page,size) => {
      const { dispatch , ID } = this.props;
      dispatch({
        type: 'ExamManager/GetTrainmanagers',
        payload: {
          id:ID,// 试卷id
          page,// 页码
          size,// 每页条数
      ...
      dispatch({
        ...
        callback: (res)=>{
          if (res.status === 'ok') {
            ...
            router.push('/exam/examManager/index');
          }else{
    ...
    deleteConfirm = (delID) => {
      const IDArr = [];
      IDArr.push(delID)
      const { dispatch , currenttrainmanagers , ID  } = this.props;
    ...
    getAddTableData = (page,size) => {
      const { dispatch , ID  } = this.props;
      dispatch({
        type: 'ExamManager/GetOtherTrainmanagers',
        payload: {
          page,// 页码
          size,// 每页条数
          id:ID,// 培训群组ID
    ...
    addHandleOk = () => {
      ...
      dispatch({
        type:'ExamManager/SubmitAddedData',
        payload: {
          id:ID,// 试卷id
          data:{
            trainmanagers:addSelectedAllKeys
          }
    ...
    render() {
        ...
        const pageConifg = {
          ...pagination,
          total:currenttrainmanagers.count,
        ...
        const dataSource = currenttrainmanagers.results.map((value)=>{
        ...
        const addDataSource = addtrainmanagers.results.map((value)=>{
          return Object.assign({},value,{key:value.id});
        });
        // ---------------增加培训管理员模态框---------------
        ...
        return (
          <PageHeaderWrapper title={`试卷编辑（${currentType}）`}>
            ...
            /> 
            <Card 
              className={styles.testPapersManagerContent}
            >
              <div className={styles.tableContent}>
                <div className={styles.searchContent}>
                  <div><span>可以使用本试卷的培训管理员：</span></div>
                  <div>
              ...
              <div className={styles.foonter_btns}>
                {
                  currentType === '拟制中'?<Button type="primary" onClick={()=>this.changeExamStatus('上架')}>上架</Button>:null              
                }
                {
                  currentType === '已下架'?<Button type="primary" onClick={()=>this.changeExamStatus('重新上架')}>重新上架</Button>:null
                }
            ...
            >
              <div className={styles.testPapersManagerContent}>
                <div style={{paddingTop:20}}>
                  <div className={styles.searchContent}>
                    <div><span>选择可以使用本试卷的培训管理员：</span><Search placeholder='输入培训管理员姓名或编号过滤' style={{width:300}} /></div>
                  </div>
                  <Table 
                    bordered 
                    dataSource={addDataSource}
                    loading={addtrainmanagersLoading}
