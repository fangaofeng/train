...
class UploadZip2 extends Component {
  ...
  getTableData = (page,size) => {
    const { dispatch } = this.props;
    dispatch({
      type: 'uploadExam/GetTrainerTableData',
      payload: {
        page,// 页码
        size,// 每页条数
      },
      ...
      <PageHeaderWrapper title='试卷上架'>
        <Card className={styles.afterUpload}>
          <Row gutter={24} className={styles.afterUploadRow}>
            <Col xs={24} sm={24} md={24} lg={24} xl={12} xxl={12} className={styles.firstCol}>
              <SelfCard title='试卷信息' bordered={false} noLeftAndRightPadding='noLeftAndRightPadding'>
                <div className={styles.imgContent}>
                  <div className={styles.imgLeft}>
                    <img src={testInfo.cover} alt="" />
                  </div>
                  <div className={styles.imgRight}>
                    <div className={styles.msgDetail}><span>试卷编号：</span><span className={styles.msgDetailOverflow} title={testInfo.number}>{testInfo.number}</span></div>
                    <div className={styles.msgDetail}><span>试卷名称：</span><span className={styles.msgDetailOverflow} title={testInfo.name}>{testInfo.name}</span></div>
                    <div className={styles.msgDetail}><span>考试时长：</span><span title={testInfo.time}>{testInfo.time}分钟</span></div>
                  </div>
                </div>
                <div className={styles.columnContent}>
                  <div className={styles.columnContentL}>
                    <div className={styles.msgDetail}><span>试卷总分：</span><span>{testInfo.score}分</span></div>
                  </div>
                  <div className={styles.columnContentR}>
                    <div className={styles.msgDetail}><span>合格分数：</span><span>{testInfo.passScore}分</span></div>
                  </div>
                </div>
                <Row>
                  <Col>
                    <div className={styles.msgDetail} style={{lineHeight:1.5}}><span>适用对象：</span><span>{testInfo.applicablePerson}</span></div>
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <div className={styles.msgDetail} style={{lineHeight:1.5}}><span>试卷介绍：</span><span>{testInfo.introduce}</span></div>
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <div className={styles.msgDetail} style={{lineHeight:1.5}}><span>适用课程编号：</span><span>{testInfo.applicableCourseNumber}</span></div>
                  </Col>
                </Row>
                <Row>
                  <Col>
                    <div className={styles.msgDetail} style={{lineHeight:1.5}}><span>适用课程名称：</span><span>{testInfo.applicableCourseName}</span></div>
                  </Col>
                </Row>
              </SelfCard>
            </Col>
        ...
        >
          <div style={{display:'flex',justifyContent:'center',alignItems:'center',padding:'20px 0'}}>
            <Icon type="check-circle" theme="filled" style={{color:'#8fc127', fontSize:'40px',marginRight:'15px'}} /><span style={{fontSize:'20px',color:'#3b3b3b'}}>试卷已保存</span>
          </div>
          <div style={{padding:'0 0 20px 0',textAlign:'center',fontSize:'16px',color:'#3b3b3b'}}>试卷已保存,您可在试卷管理页中查看已保存试卷！</div>
        </Modal>
      </PageHeaderWrapper>
