...
export async function getOnlineExamBasicInfo(params) {
  return request(`/exam/progress/${params.id}`);
}
...
export async function getExamQuestions(params) {
  if (params.page) {
    ...
    return request(`/exam/progress/${params.id}/questions?${stringify(queryparams)}`);
  }
  return request(`/exam/progress/${params.id}/questions`);
}
