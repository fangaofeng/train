...
const getIcon = icon => {
  ...
  // 自定义icon图标
  if (typeof icon === 'string' && icon.indexOf('self') === 0) {
    const iconname = icon.split('_');
    return SVGIcon[iconname[1]];
  }
  if (typeof icon === 'string' && icon.indexOf('self') === -1) {
